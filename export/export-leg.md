...
**EN:** A modern packaging form for Tcl code: a single `.tm` file with a version-encoded filename (e.g. `mypkg-1.2.tm`) that is loadable via `package require` without a separate `pkgIndex.tcl`. Searched on directories listed by `tcl::tm::path list`. Simpler than the classic pkgIndex mechanism for single-file packages.

**DE:** Eine moderne Paketform für Tcl-Code: eine einzelne `.tm`-Datei mit versions-kodiertem Dateinamen (z.B. `mypkg-1.2.tm`), die via `package require` ohne separate `pkgIndex.tcl` ladbar ist. Wird in den Verzeichnissen aus `tcl::tm::path list` gesucht. Einfacher als der klassische pkgIndex-Mechanismus für Single-File-Pakete.

**Example (EN):**
```tcl
# Filename convention: <name>-<version>.tm
# Example: mypkg-1.2.tm
...
tcl::tm::path add /opt/tcllibs
package require mypkg
```

**Related terms:** tcl::tm::path, tcl::tm::add, tcl::tm::list, package require, pkgIndex.tcl

**Siehe auch:** tm(n), package(n)

---

### auto_index

**EN:** A global array maintained by the `auto_load` mechanism that maps command names to the Tcl files providing them. Populated from `tclIndex` files generated by `auto_mkindex`. When the `unknown` handler intercepts an undefined command, it looks up `auto_index($cmd)` and sources the listed file on demand.

**DE:** Ein globales Array, das vom `auto_load`-Mechanismus gepflegt wird und Kommando-Namen auf die Tcl-Dateien abbildet, die sie bereitstellen. Wird aus `tclIndex`-Dateien gefüllt, die mit `auto_mkindex` generiert werden. Wenn der `unknown`-Handler ein nicht definiertes Kommando abfängt, schlägt er in `auto_index($cmd)` nach und lädt die entsprechende Datei on-demand.

**Example (EN):**
```tcl
# Generate tclIndex in current dir from all *.tcl:
auto_mkindex . *.tcl

# Inspect what's known:
parray auto_index

# auto_index({myProc}) -> source path/to/myProc.tcl
```

**Related terms:** unknown, auto_load, auto_mkindex, tclIndex

**Siehe auch:** library(n), auto_mkindex(n)
...
**EN:** Compares two Tcl version strings according to Tcl's version-numbering rules. Returns -1 if the first is older, 0 if equal, 1 if newer. Aware of the major.minor.patch.alpha/beta convention (`1.2a3` < `1.2b1` < `1.2.0`).

**DE:** Vergleicht zwei Tcl-Versions-Strings gemäß der Tcl-Versionsregeln. Liefert -1 wenn der erste älter ist, 0 bei Gleichheit, 1 wenn neuer. Berücksichtigt die major.minor.patch.alpha/beta-Konvention (`1.2a3` < `1.2b1` < `1.2.0`).

**Example (EN):**
```tcl
package vcompare 1.0 1.1        ;# -1 (older)
package vcompare 2.0 2.0        ;#  0 (equal)
package vcompare 2.1 2.0.5      ;#  1 (newer)
package vcompare 1.2a3 1.2b1    ;# -1 (alpha before beta)

# Conditional based on Tcl version
if {[package vcompare [info patchlevel] 8.6] >= 0} {
    # We're on 8.6 or later
}
```

**Related terms:** package versions, package require, info patchlevel

**Siehe auch:** package(n)

---

### package versions

**EN:** Returns a list of all versions of a given package that are known to the interpreter (registered via `package ifneeded` or already loaded), without loading anything. Useful for build scripts that need to pick a specific version, or for diagnostics when `package require` fails.

**DE:** Liefert eine Liste aller Versionen eines Pakets, die dem Interpreter bekannt sind (registriert via `package ifneeded` oder bereits geladen), ohne etwas zu laden. Nützlich für Build-Skripte, die eine bestimmte Version wählen müssen, oder zur Diagnose, wenn `package require` fehlschlägt.

**Example (EN):**
```tcl
# What Tcl extension versions are available?
foreach pkg {Tcl Tk sqlite3 tdbc::sqlite3} {
    puts "$pkg: [package versions $pkg]"
}

# Pick the newest available
set versions [package versions sqlite3]
set newest [lindex [lsort -command [list package vcompare] $versions] end]
package require sqlite3 $newest
```

**Related terms:** package vcompare, package require, package present, package ifneeded
...
**EN:** Adds one or more directories to the front of the Tcl Module search path. Newly added directories take precedence over previously known ones. Equivalent to `tcl::tm::path add ...` (the `path` subcommand is implicit at this level since Tcl 8.5).

**DE:** Fügt ein oder mehrere Verzeichnisse vorne in den Tcl-Module-Suchpfad ein. Neu hinzugefügte Verzeichnisse haben Vorrang vor zuvor bekannten. Äquivalent zu `tcl::tm::path add ...` (der `path`-Sub-Befehl ist auf dieser Ebene seit Tcl 8.5 implizit).

**Example (EN):**
```tcl
# Add a project-local module directory
tcl::tm::path add [file join [file dirname [info script]] tm]

# Multiple at once
tcl::tm::path add /opt/tcllibs ~/lib/tcl/tm

# Verify
puts [tcl::tm::path list]
```

**Related terms:** tcl::tm::path, tcl::tm::list, Tcl Module (.tm), auto_path
...
**EN:** Returns the current list of Tcl Module search directories, in the order they will be probed. Useful for diagnostics (`why was my .tm not found?`) and for verifying that startup scripts have set up the path correctly. Same as `tcl::tm::path list`.

**DE:** Liefert die aktuelle Liste der Tcl-Module-Suchverzeichnisse, in der Reihenfolge, in der sie durchsucht werden. Nützlich zur Diagnose (`warum wurde mein .tm nicht gefunden?`) und zur Verifikation, dass Startup-Skripte den Pfad korrekt gesetzt haben. Identisch mit `tcl::tm::path list`.
...
# Default paths typically include:
# /usr/local/lib/tcl9.0/tcl9
# /usr/local/share/tcl9.0/tcl9
# ~/.local/lib/tcl9/tcl9   (per Tcl version)
```

**Related terms:** tcl::tm::path, tcl::tm::add, Tcl Module (.tm), auto_path
...
**EN:** Windows clipboard format ID 2 — a device-dependent bitmap (HBITMAP handle). Together with `CF_DIB` (format 8, device-independent bitmap) the primary way images travel through the clipboard on Windows. TWAPI exposes raw access; for Tk's photo image, a small bridge is required to convert between HBITMAP and Tk's image format.

**DE:** Windows-Clipboard-Format-ID 2 — eine gerätenabhängige Bitmap (HBITMAP-Handle). Zusammen mit `CF_DIB` (Format 8, geräteunabhängige Bitmap) der primäre Weg, Bilder über die Zwischenablage zu transportieren. TWAPI bietet rohen Zugriff; für Tks photo-image ist ein kleiner Adapter nötig, um zwischen HBITMAP und Tks Image-Format zu konvertieren.
...
set hasBmp  [twapi::clipboard_format_available 2]
set hasDib  [twapi::clipboard_format_available 8]
puts "Bitmap: $hasBmp   DIB: $hasDib"

# Available formats overall
foreach fmt [twapi::get_clipboard_formats] {
    puts "format $fmt"
}
```

**Related terms:** CF_TEXT, CF_UNICODETEXT, clipboard, CF_DIB, HBITMAP

**Siehe auch:** twapi_clipboard(n)

---

### CF_TEXT

**EN:** Windows clipboard format ID 1 — plain ANSI text (single-byte, system code page). The most basic clipboard format; almost every application offering text exposes this. Modern apps prefer `CF_UNICODETEXT` because it preserves characters outside the local code page. TWAPI: `twapi::clipboard_format_available 1` and `twapi::read_clipboard 1`.

**DE:** Windows-Clipboard-Format-ID 1 — reiner ANSI-Text (Single-Byte, System-Code-Page). Das einfachste Clipboard-Format; nahezu jede Text-anbietende Anwendung stellt es bereit. Moderne Apps bevorzugen `CF_UNICODETEXT`, weil es Zeichen außerhalb der lokalen Code-Page bewahrt. TWAPI: `twapi::clipboard_format_available 1` und `twapi::read_clipboard 1`.

**Example (EN):**
```tcl
package require twapi

# Check whether plain text is on the clipboard
if {[twapi::clipboard_format_available 1]} {
    set text [twapi::read_clipboard 1]
    puts "Got [string length $text] characters of ANSI text"
}
```

**Related terms:** CF_UNICODETEXT, CF_BITMAP, clipboard, TWAPI, Windows Code Pages
...
**EN:** Windows clipboard format ID 13 — UTF-16LE text. The preferred format for modern apps because it preserves all Unicode characters regardless of the system code page. Tcl's built-in `clipboard get` returns content that came in as `CF_UNICODETEXT` correctly; raw byte-level access is available via `twapi::read_clipboard 13`.

**DE:** Windows-Clipboard-Format-ID 13 — UTF-16LE-Text. Das bevorzugte Format für moderne Apps, weil es alle Unicode-Zeichen unabhängig von der System-Code-Page bewahrt. Tcls eingebauter `clipboard get` liefert als `CF_UNICODETEXT` eingegangenen Inhalt korrekt; rohen Byte-Zugriff bietet `twapi::read_clipboard 13`.

**Example (EN):**
```tcl
package require twapi

# Modern preferred format
if {[twapi::clipboard_format_available 13]} {
    set text [twapi::read_clipboard 13]
    # Already a Tcl string -- emoji and CJK work
    puts $text
}

# Equivalent via Tk's built-in (cross-platform):
if {![catch {clipboard get -type STRING} text]} {
    puts $text
}
```

**Related terms:** CF_TEXT, CF_BITMAP, clipboard, UTF-16LE, TWAPI

**Siehe auch:** twapi_clipboard(n), clipboard(n)
...
**Related terms:** fconfigure, fconfigure -translation, encoding system, encoding convertfrom

**Siehe auch:** fconfigure(n), open(n)
...
**Related terms:** fconfigure, fconfigure -encoding, open, gets, puts
...
**EN:** Returns the list of math functions known to `expr` (e.g. `sin`, `cos`, `sqrt`, `pow`, `abs`, …). With a glob pattern, returns only matching names. Useful for documentation tools and for checking whether a function exists before using it.

**DE:** Liefert die Liste der Mathe-Funktionen, die `expr` kennt (z.B. `sin`, `cos`, `sqrt`, `pow`, `abs`, …). Mit einem Glob-Pattern werden nur passende Namen zurückgegeben. Nützlich für Dokumentations-Tools und um zu prüfen, ob eine Funktion existiert, bevor sie genutzt wird.

**Example (EN):**
```tcl
# List everything
info functions       ;# abs acos asin atan ...

# Only those starting with 'a'
info functions a*    ;# abs acos asin atan

# Existence check
if {"sinh" in [info functions]} {
    puts "sinh available"
}
```

**Related terms:** expr, info commands, info procs, tcl::mathfunc

**Siehe auch:** info(n), expr(n), mathfunc(n)
...
**EN:** Returns a list of packages that have been dynamically loaded into the current or a child interpreter via `load` (typically shared C libraries with Tcl extensions). Each entry is a pair `{filename packageName}`. Empty for pure-Tcl packages loaded via `source` or `package require`.

**DE:** Liefert eine Liste der Pakete, die dynamisch in den aktuellen oder einen Sub-Interpreter via `load` geladen wurden (typischerweise Shared C-Libraries mit Tcl-Erweiterungen). Jeder Eintrag ist ein Paar `{Dateiname Paketname}`. Leer für reine Tcl-Pakete, die via `source` oder `package require` geladen wurden.

**Example (EN):**
```tcl
# What C extensions are currently linked in?
foreach pair [info loaded] {
    lassign $pair file pkg
    puts "$pkg loaded from $file"
}
# Example output:
# Sqlite loaded from /usr/lib/tcltk/sqlite3.36.0/libtclsqlite3.so
# Tk     loaded from /usr/lib/tcltk/x86_64-linux-gnu/libtk8.6.so
```

**Related terms:** load, info library, info sharedlibextension, package present

**Siehe auch:** info(n), load(n)
...
**Related terms:** regsub, regexp, apply, lambda
...
**EN:** Version 1.2 of Transport Layer Security (2008, RFC 5246) — the minimum acceptable TLS version for current Internet traffic. The TLS extension enables it via the `-tls1.2 1` socket option, often together with `-tls1.3 1`. Older versions (TLS 1.0 / 1.1, SSL 3.0) are insecure and should be disabled with `-tls1 0 -tls1.1 0 -ssl3 0`.

**DE:** Version 1.2 von Transport Layer Security (2008, RFC 5246) — die minimal akzeptable TLS-Version für aktuellen Internet-Verkehr. Die TLS-Erweiterung aktiviert sie über die Socket-Option `-tls1.2 1`, oft zusammen mit `-tls1.3 1`. Ältere Versionen (TLS 1.0 / 1.1, SSL 3.0) sind unsicher und sollten mit `-tls1 0 -tls1.1 0 -ssl3 0` deaktiviert werden.

**Example (EN):**
```tcl
package require tls

# Modern secure baseline
tls::init \
...
set sock [tls::socket api.example.com 443]
# After handshake, query the negotiated version:
set info [tls::status $sock]
puts "Negotiated: [dict get $info protocol]"   ;# e.g. TLSv1.2 or TLSv1.3
```

**Related terms:** TLS 1.3, tls::socket, tls::init, tls::status, http::register

**Siehe auch:** tls(n), RFC 5246

---

### TLS 1.3

**EN:** Version 1.3 of Transport Layer Security (2018, RFC 8446) — the current state of the art. Removes weak ciphers, simplifies the handshake to a single round-trip (1-RTT) and supports 0-RTT resumption. Enabled in the TLS extension via `-tls1.3 1`. Requires a sufficiently recent OpenSSL build underneath (1.1.1 or later); older tcltls/openssl combinations may silently fall back to TLS 1.2.

**DE:** Version 1.3 von Transport Layer Security (2018, RFC 8446) — der aktuelle Stand der Technik. Entfernt schwache Cipher, vereinfacht den Handshake auf eine einzige Round-Trip (1-RTT) und unterstützt 0-RTT-Resumption. Wird in der TLS-Erweiterung über `-tls1.3 1` aktiviert. Setzt einen ausreichend aktuellen OpenSSL-Build voraus (1.1.1 oder neuer); ältere tcltls/openssl-Kombinationen fallen still auf TLS 1.2 zurück.
...
**Related terms:** TLS 1.2, tls::socket, tls::init, tls::status, OpenSSL

**Siehe auch:** tls(n), RFC 8446
...
**EN:** Registers a transport handler for a URL scheme with the `http` package. The canonical use is binding `https` to a TLS socket factory so that subsequent `http::geturl https://...` calls transparently establish TLS. The handler must be a command that returns a socket — typically `::tls::socket` plus options.

**DE:** Registriert für ein URL-Schema einen Transport-Handler beim `http`-Paket. Die kanonische Verwendung ist die Bindung von `https` an einen TLS-Socket-Factory, damit folgende `http::geturl https://...`-Aufrufe transparent eine TLS-Verbindung aufbauen. Der Handler muss ein Kommando sein, das einen Socket zurückgibt — typischerweise `::tls::socket` plus Optionen.
...
# Configure TLS once, register, then use http normally
tls::init -tls1.2 1 -tls1.3 1 \
          -cafile /etc/ssl/certs/ca-certificates.crt \
          -require 1
...
# Equivalent with options passed inline
http::register https 443 [list ::tls::socket -require 1 \
    -cafile /etc/ssl/certs/ca-certificates.crt]
```

**Related terms:** tls::socket, TLS 1.2, TLS 1.3, http::geturl

**Siehe auch:** http(n), tls(n)
...
**EN:** Socket-creation command from the TLS extension that wraps a TCP socket in TLS/SSL. Drop-in replacement for the built-in `socket` command — accepts the same -async / -server / host / port arguments plus TLS-specific options like -tls1.2, -tls1.3, -cafile, -certfile, -require, -command. Returned channel is read/written like any Tcl socket; the TLS handshake happens transparently on first I/O (or explicitly via `tls::handshake`).

**DE:** Socket-Erzeugungs-Kommando der TLS-Erweiterung, das einen TCP-Socket in TLS/SSL einpackt. Drop-in-Ersatz für das eingebaute `socket`-Kommando — nimmt die gleichen -async / -server / host / port Argumente plus TLS-spezifische Optionen wie -tls1.2, -tls1.3, -cafile, -certfile, -require, -command. Der zurückgegebene Channel wird wie jeder Tcl-Socket gelesen/geschrieben; der TLS-Handshake findet beim ersten I/O transparent statt (oder explizit via `tls::handshake`).
...
**Related terms:** http::register, TLS 1.2, TLS 1.3, socket, fconfigure

**Siehe auch:** tls(n), https://core.tcl-lang.org/tcltls/
...
**Related terms:** Codepoint, scan %c, format
...
**Related terms:** Codepoint, format %c, scan
...
**Related terms:** scan, format %c, Codepoint
...
**EN:** Element of the `tcl_platform` array giving the login name of the user running the script. On Unix the value of `$USER` or the result of `getpwuid(getuid())`; on Windows the value from `GetUserName`. Preferred over `env(USER)` or `env(USERNAME)` because it is portable across platforms.

**DE:** Element des `tcl_platform`-Arrays mit dem Login-Namen des Benutzers, der das Skript ausführt. Auf Unix der Wert von `$USER` oder das Ergebnis von `getpwuid(getuid())`; auf Windows der Wert aus `GetUserName`. Bevorzugt gegenüber `env(USER)` oder `env(USERNAME)`, weil plattformübergreifend portabel.

**Example (EN):**
```tcl
puts "Hello, $::tcl_platform(user)!"

# Per-user data directory (portable)
set dataDir [file join $::env(HOME) .myapp $::tcl_platform(user)]
file mkdir $dataDir
```

**Related terms:** tcl_platform, tcl_platform(platform), env

**Siehe auch:** tclvars(n)
...
## Windows-Plattform und TWAPI / Windows Platform and TWAPI

### CSIDL

**EN:** Constant Special Item ID List — a numeric identifier for a well-known Windows folder (Desktop, Documents, AppData, ProgramFiles, ...). Used by `twapi::get_shell_folder` and the underlying `SHGetFolderPath` API. The newer KNOWNFOLDERID GUIDs supersede CSIDL on Vista+, but CSIDL constants still work via TWAPI for compatibility.

**DE:** Constant Special Item ID List — eine numerische ID für einen bekannten Windows-Ordner (Desktop, Dokumente, AppData, ProgramFiles, ...). Wird von `twapi::get_shell_folder` und der zugrunde liegenden `SHGetFolderPath`-API genutzt. Die neueren KNOWNFOLDERID-GUIDs lösen CSIDL ab Vista+ ab, aber CSIDL-Konstanten funktionieren via TWAPI weiterhin aus Kompatibilitätsgründen.
...
# Common CSIDLs
puts [twapi::get_shell_folder CSIDL_DESKTOP]
puts [twapi::get_shell_folder CSIDL_PERSONAL]    ;# Documents
puts [twapi::get_shell_folder CSIDL_APPDATA]     ;# Roaming AppData
puts [twapi::get_shell_folder CSIDL_LOCAL_APPDATA]
puts [twapi::get_shell_folder CSIDL_PROGRAM_FILES]
puts [twapi::get_shell_folder CSIDL_STARTUP]
```

**Related terms:** TWAPI, shell_execute, KNOWNFOLDERID

**Siehe auch:** twapi_shell(n)

---

### Filesystem Watcher

**EN:** A facility to receive notifications when files in a directory tree change (created, modified, deleted, renamed). On Windows wrapped by TWAPI via `ReadDirectoryChangesW`; the Tcl-side proc is `twapi::begin_filesystem_monitor`. The supplied callback runs in the Tcl event loop. Cancel monitoring with `twapi::cancel_filesystem_monitor`.

**DE:** Eine Einrichtung, um Benachrichtigungen zu erhalten, wenn sich Dateien in einem Verzeichnisbaum ändern (erstellt, modifiziert, gelöscht, umbenannt). Auf Windows von TWAPI über `ReadDirectoryChangesW` gekapselt; die Tcl-seitige Proc heißt `twapi::begin_filesystem_monitor`. Der übergebene Callback läuft in der Tcl-Eventloop. Überwachung beenden mit `twapi::cancel_filesystem_monitor`.
...
**Related terms:** TWAPI, ReadDirectoryChangesW, vwait, Event Loop

**Siehe auch:** twapi_filesystem(n)

---

### HWND

**EN:** Window Handle — an opaque integer identifier used by the Win32 API to refer to a window (anything from a top-level frame to a button). Returned by TWAPI procs like `twapi::find_windows` and accepted by `twapi::get_window_text`, `twapi::move_window`, `twapi::set_foreground_window`. Not portable: HWNDs are valid only within a single Windows session.

**DE:** Window Handle — eine opake Integer-ID, mit der die Win32-API auf ein Fenster verweist (von Top-Level-Frame bis Button). Wird von TWAPI-Procs wie `twapi::find_windows` zurückgegeben und von `twapi::get_window_text`, `twapi::move_window`, `twapi::set_foreground_window` akzeptiert. Nicht portabel: HWNDs sind nur innerhalb einer Windows-Sitzung gültig.
...
**Related terms:** TWAPI, find_windows, Win32, tcl_platform(platform)

**Siehe auch:** twapi_windows(n)

---

### Magicsplat

**EN:** A current free distribution of Tcl/Tk for Windows, maintained by Ashok Nadkarni. Successor to the role ActiveState's ActiveTcl previously played. Ships Tcl 8.6 / 9.x, Tk, Tcllib, Tklib, tdom, TWAPI, sqlite3, tcom, and ~50 other extensions in one installer. The standard recommendation for Windows users today.

**DE:** Eine aktuelle freie Tcl/Tk-Distribution für Windows, gepflegt von Ashok Nadkarni. Übernimmt die Rolle, die früher ActiveState's ActiveTcl spielte. Liefert Tcl 8.6 / 9.x, Tk, Tcllib, Tklib, tdom, TWAPI, sqlite3, tcom und ~50 weitere Erweiterungen in einem Installer. Die Standardempfehlung für Windows-Nutzer heutzutage.

**Example (EN):**
```tcl
# After installation from https://www.magicsplat.com/tcl-installer/
# the standard packages are available via package require:
...
# All ready to go, no separate downloads needed.
```

**Related terms:** TWAPI, ActiveTcl, teacup, Teapot, tcom

**Siehe auch:** https://www.magicsplat.com/

---

### SID (Security Identifier)

**EN:** Security Identifier — a unique value identifying a Windows user account, group, or computer (e.g. `S-1-5-21-...`). Used throughout Win32 security APIs for access-control entries (ACEs), token identity and audit records. TWAPI returns SIDs as strings (the SDDL form) and provides conversion helpers like `twapi::lookup_account_sid` (SID → name) and `twapi::lookup_account_name` (name → SID).

**DE:** Security Identifier — ein eindeutiger Wert, der ein Windows-Benutzerkonto, eine Gruppe oder einen Computer identifiziert (z.B. `S-1-5-21-...`). Wird in der gesamten Win32-Security-API für Access-Control-Entries (ACEs), Token-Identitäten und Audit-Records genutzt. TWAPI gibt SIDs als Strings (in SDDL-Form) zurück und stellt Konvertierungs-Helper bereit wie `twapi::lookup_account_sid` (SID → Name) und `twapi::lookup_account_name` (Name → SID).
...
puts $sid
# => S-1-5-21-1234567890-987654321-555555555-1001

# SID -> account name
array set info [twapi::lookup_account_sid $sid]
puts "$info(domain)\\$info(name)"

# Account name -> SID
set adminSid [twapi::lookup_account_name Administrators]
```

**Related terms:** TWAPI, tcl_platform(user), Win32 Security

**Siehe auch:** twapi_security(n)

---

### TWAPI

**EN:** Tcl Windows API — an extensive Tcl extension that exposes hundreds of Win32 API functions to Tcl scripts (processes, services, registry, network adapters, clipboard, shell, COM, security). Bundled with Magicsplat Tcl/Tk and installable via `package require twapi`. Replaces the need for C glue code or external command-line tools on Windows.

**DE:** Tcl Windows API — eine umfangreiche Tcl-Erweiterung, die hunderte Win32-API-Funktionen für Tcl-Skripte verfügbar macht (Prozesse, Dienste, Registry, Netzwerk-Adapter, Zwischenablage, Shell, COM, Security). Bei Magicsplat Tcl/Tk dabei, ladbar via `package require twapi`. Ersetzt unter Windows die Notwendigkeit von C-Glue-Code oder externen CLI-Tools.
...
puts "RAM: [expr {[dict get $mem -totalphys] / 1024.0 / 1024 / 1024}] GB"

# Find Notepad windows
foreach hwnd [twapi::find_windows -text "*Notepad*"] {
    puts "[twapi::get_window_text $hwnd]"
}
```

**Related terms:** Magicsplat, tcl_platform(platform), registry, COM (Component Object Model), HWND

**Siehe auch:** https://twapi.magicsplat.com/, twapi(n)

---

### Teapot

**EN:** Tcl Extension Archive Pot — a network-accessible repository of pre-built Tcl extensions for multiple platforms, originally hosted by ActiveState. Client tool: `teacup`. Each entry is a `.tap` or `.zip` containing platform-specific binaries plus metadata (package name, version, architecture, dependencies). Modern projects increasingly bundle dependencies directly (Starkit, ZipKit) instead of relying on Teapot.

**DE:** Tcl Extension Archive Pot — ein netzwerk-erreichbares Repository vorgebauter Tcl-Erweiterungen für mehrere Plattformen, ursprünglich von ActiveState gehostet. Client-Tool: `teacup`. Jeder Eintrag ist ein `.tap` oder `.zip` mit plattformspezifischen Binaries plus Metadaten (Paketname, Version, Architektur, Abhängigkeiten). Moderne Projekte bündeln Abhängigkeiten zunehmend direkt (Starkit, ZipKit) statt auf Teapot zu setzen.

**Example (EN):**
```tcl
# A teapot URL
# http://teapot.activestate.com/package/name/twapi/ver/4.4.7/arch/win32-ix86

# Use teacup as the client
teacup install twapi
# (Resolves dependencies via the repository's metadata)
```

**Related terms:** teacup, ActiveTcl, Magicsplat, Starkit, ZipKit
...
**EN:** Tcl built-in command on Windows for reading and writing the Windows Registry. Loaded via `package require registry`. Provides `registry get`, `registry set`, `registry delete`, `registry keys`, `registry values`, `registry type`. Hive names can be abbreviated: `HKLM` for `HKEY_LOCAL_MACHINE`, `HKCU` for `HKEY_CURRENT_USER`.

**DE:** In Tcl auf Windows eingebauter Befehl zum Lesen und Schreiben der Windows-Registry. Wird via `package require registry` geladen. Bietet `registry get`, `registry set`, `registry delete`, `registry keys`, `registry values`, `registry type`. Hive-Namen können abgekürzt werden: `HKLM` für `HKEY_LOCAL_MACHINE`, `HKCU` für `HKEY_CURRENT_USER`.
...
# Read a value
set shellState [registry get \
    {HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer} \
    ShellState]

# Enumerate subkeys
set subkeys [registry keys HKCU\\Software]

# Write a value (DWORD)
registry set HKCU\\Software\\MyApp Version 1 dword

# Delete a value
registry delete HKCU\\Software\\MyApp Version
```

**Related terms:** TWAPI, tcl_platform(platform), HKEY_LOCAL_MACHINE, HKEY_CURRENT_USER

**Siehe auch:** registry(n)

---

### teacup

**EN:** Command-line client for the Teapot package repository (originally ActiveState, now community-mirrored). Installs Tcl extensions on demand into a local Teapot directory. Largely superseded by Magicsplat's bundled approach, but still used in legacy ActiveTcl environments and for CI scripts. Syntax: `teacup install <pkg>`, `teacup search <term>`, `teacup list`.

**DE:** Command-Line-Client für das Teapot-Package-Repository (ursprünglich ActiveState, heute community-gespiegelt). Installiert Tcl-Erweiterungen on-demand in ein lokales Teapot-Verzeichnis. Größtenteils durch Magicsplat's Bundle-Ansatz abgelöst, wird aber noch in Legacy-ActiveTcl-Umgebungen und CI-Skripten genutzt. Syntax: `teacup install <pkg>`, `teacup search <term>`, `teacup list`.
...
teacup search sqlite

# List what's installed locally
teacup list
```

**Related terms:** Teapot, ActiveTcl, Magicsplat, package require
...
**EN:** Microsoft's binary interface standard for software components, the foundation for OLE, ActiveX, and DCOM. A COM object exposes one or more interfaces (sets of methods identified by IIDs) and is identified by a CLSID or a human-readable ProgID like `Excel.Application`. In Tcl, COM objects are accessed via the `tcom` extension or TWAPI's COM bindings.

**DE:** Microsofts binärer Interface-Standard für Software-Komponenten, Basis für OLE, ActiveX und DCOM. Ein COM-Objekt stellt eine oder mehrere Interfaces bereit (Methoden-Mengen, identifiziert durch IIDs) und wird über eine CLSID oder eine menschenlesbare ProgID wie `Excel.Application` referenziert. In Tcl werden COM-Objekte über die `tcom`-Erweiterung oder TWAPI's COM-Bindings angesprochen.
...
**Related terms:** tcom, OLE Automation, ActiveX, DCOM, WMI
...
**EN:** A subset of COM that exposes scriptable object models — designed so that interpreted languages (originally Visual Basic, JScript, also Tcl via tcom) can drive Windows applications. All Office applications, Internet Explorer, WSH, ADO, WMI etc. are accessible through OLE Automation. Methods, properties and events are described in type libraries (`.tlb`).

**DE:** Ein Subset von COM, das skriptbare Object-Models bereitstellt — entworfen, damit interpretierte Sprachen (ursprünglich Visual Basic, JScript, auch Tcl via tcom) Windows-Anwendungen steuern können. Alle Office-Anwendungen, Internet Explorer, WSH, ADO, WMI usw. sind über OLE-Automation erreichbar. Methoden, Properties und Events sind in Type Libraries (`.tlb`) beschrieben.

**Example (EN):**
```tcl
package require tcom

# Drive Word via OLE Automation
set word [::tcom::ref createobject "Word.Application"]
$word Visible 1

set doc [[$word Documents] Add]
set selection [$word Selection]
$selection TypeText "Generated by Tcl on [clock format [clock seconds]]"
$selection TypeParagraph

# Property access vs. method:
set name [$doc Name]              ;# property read
$doc SaveAs C:/Temp/out.docx      ;# method call
```

**Related terms:** COM (Component Object Model), tcom, ActiveX, WSH
...
**EN:** A Tcl extension for COM/OLE Automation on Windows. Lets Tcl scripts create and drive ActiveX / COM objects (Excel, Word, Internet Explorer, Outlook, WMI, ADO, ...). Loaded via `package require tcom`. The main entry point is `::tcom::ref createobject "Class.ProgID"`, after which methods are called as if on a Tcl command.

**DE:** Eine Tcl-Erweiterung für COM/OLE-Automation auf Windows. Erlaubt Tcl-Skripten, ActiveX-/COM-Objekte zu erzeugen und zu steuern (Excel, Word, Internet Explorer, Outlook, WMI, ADO, ...). Wird via `package require tcom` geladen. Haupteinstieg: `::tcom::ref createobject "Class.ProgID"`, danach werden Methoden wie auf einem Tcl-Kommando aufgerufen.
...
[$sheet Cells 1 1] Value "Hello from Tcl!"
...
**Related terms:** COM (Component Object Model), OLE Automation, TWAPI, Magicsplat

**Siehe auch:** http://www.vex.net/~cthuang/tcom/
...
**Related terms:** Codepoint, Supplementary Plane, Surrogate Pair

**Siehe auch:** Tcl_UniChar(3)
...
**Related terms:** BMP, Supplementary Plane, format %c, scan %c

**Siehe auch:** Tcl_UniChar(3), encoding(n)
...
**Related terms:** BMP, Codepoint, Surrogate Pair, Tcl/Tk 9
...
**Related terms:** BMP, Supplementary Plane, UTF-16
...
**Related terms:** UTF-16, UTF-16LE, encoding convertfrom, encoding convertto

**Siehe auch:** encoding(n)
...
**Related terms:** UTF-16, UTF-16BE, encoding convertfrom, encoding convertto
...
**Related terms:** encoding names, encoding system, encoding convertfrom
