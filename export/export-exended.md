...
Source database: `/home/greg/lib/tcltk/tcltk-glossary/glossary.db`  
Exported: 2026-05-15T17:57:05Z  
Terms: 1722
...
```
- RELATED: tcl::tm::path, tcl::tm::add, tcl::tm::list, package require, pkgIndex.tcl
- SEE: tm(n), package(n)
...
```
- RELATED: unknown, auto_load, auto_mkindex, tclIndex
- SEE: library(n), auto_mkindex(n)
...
```
- RELATED: package versions, package require, info patchlevel
- SEE: package(n)
...
```
- RELATED: package vcompare, package require, package present, package ifneeded
- SEE: package(n)
...
```
- RELATED: tcl::tm::path, tcl::tm::list, Tcl Module (.tm), auto_path
- SEE: tm(n)
...
```
- RELATED: tcl::tm::path, tcl::tm::add, Tcl Module (.tm), auto_path
- SEE: tm(n)
...
```
- RELATED: CF_TEXT, CF_UNICODETEXT, clipboard, CF_DIB, HBITMAP
- SEE: twapi_clipboard(n)
...
```
- RELATED: CF_UNICODETEXT, CF_BITMAP, clipboard, TWAPI, Windows Code Pages
- SEE: twapi_clipboard(n)
...
```
- RELATED: CF_TEXT, CF_BITMAP, clipboard, UTF-16LE, TWAPI
- SEE: twapi_clipboard(n), clipboard(n)
...
```
- RELATED: expr, info commands, info procs, tcl::mathfunc
- SEE: info(n), expr(n), mathfunc(n)
...
```
- RELATED: load, info library, info sharedlibextension, package present
- SEE: info(n), load(n)
...
```
- RELATED: TLS 1.3, tls::socket, tls::init, tls::status, http::register
- SEE: tls(n), RFC 5246
...
close $sock
```
- RELATED: TLS 1.2, tls::socket, tls::init, tls::status, OpenSSL
- SEE: tls(n), RFC 8446
...
http::register https 443 [list ::tls::socket -require 1 \
    -cafile /etc/ssl/certs/ca-certificates.crt]
```
- RELATED: tls::socket, TLS 1.2, TLS 1.3, http::geturl
- SEE: http(n), tls(n)
...
http::register https 443 ::tls::socket
```
- RELATED: http::register, TLS 1.2, TLS 1.3, socket, fconfigure
- SEE: tls(n), https://core.tcl-lang.org/tcltls/
...
file mkdir $dataDir
```
- RELATED: tcl_platform, tcl_platform(platform), env
- SEE: tclvars(n)
...
- RELATED: pack forget, grid forget, winfo exists
...
```
- RELATED: TWAPI, shell_execute, KNOWNFOLDERID
- SEE: twapi_shell(n)
...
```
- RELATED: TWAPI, ReadDirectoryChangesW, vwait, Event Loop
- SEE: twapi_filesystem(n)
...
```
- RELATED: TWAPI, find_windows, Win32, tcl_platform(platform)
- SEE: twapi_windows(n)
...
```
- RELATED: TWAPI, ActiveTcl, teacup, Teapot, tcom
- SEE: https://www.magicsplat.com/
...
```
- RELATED: TWAPI, tcl_platform(user), Win32 Security
- SEE: twapi_security(n)
...
```
- RELATED: Magicsplat, tcl_platform(platform), registry, COM (Component Object Model), HWND
- SEE: https://twapi.magicsplat.com/, twapi(n)
...
```
- RELATED: teacup, ActiveTcl, Magicsplat, Starkit, ZipKit
- SEE: teacup(1)
...
```
- RELATED: TWAPI, tcl_platform(platform), HKEY_LOCAL_MACHINE, HKEY_CURRENT_USER
- SEE: registry(n)
...
teacup list
```
- RELATED: Teapot, ActiveTcl, Magicsplat, package require
- SEE: teacup(1)
...
```
- RELATED: tcom, OLE Automation, ActiveX, DCOM, WMI
- SEE: tcom(n)
...
```
- RELATED: COM (Component Object Model), tcom, ActiveX, WSH
- SEE: tcom(n)
...
$excel Quit
```
- RELATED: COM (Component Object Model), OLE Automation, TWAPI, Magicsplat
